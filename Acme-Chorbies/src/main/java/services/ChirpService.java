...
public class ChirpService {
	...
	public void broadcastChirp(ChirpForm chirpForm) {
		Chirp chirp;
		Collection<Chorbi> listChorbi = this.chorbiService.getChorbiesByEvent(chirpForm.getEventId());
	...
	public Chirp applyPrivacity(final Chirp chirp) {
		...
		if (chirp.getText() != null && !chirp.getText().isEmpty()) {
		...
		if (chirp.getSubject() != null && !chirp.getSubject().isEmpty()) {
		...
		if (chirp.getAttachments() != null && !chirp.getAttachments().isEmpty()) {
	...
	public void createDefaultEventChirp(final boolean edited, final Event event) {
		...
		for (final Chorbi chorbi : chorbies) {
			if (edited == true) {
				...
				chirp.setSubject("The event: " + event.getTitle() + " was cancelled");
